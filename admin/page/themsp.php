...
if(isset($_POST['themsp'])){
    ...
    if(!empty($_POST['tensp1']) && isset($_POST['theloai']) && !empty($_POST['theloai']) && isset($_POST['giasp']) && !empty($_POST['giasp']) && isset($_POST['sl']) && !empty($_POST['sl'])){
        ...
        $hinhanh1=$_FILES['upload_file1']['name'];
        $hinhanh2=$_FILES['upload_file2']['name'];
        $hinhanh3=$_FILES['upload_file3']['name'];
        $hinhanh4=$_FILES['upload_file4']['name'];
        $tensp1=$_POST['tensp1'];
        $sql="INSERT INTO `tbl_products`(`name`, `price`, `details`, `status`, `image`,`image2`,`image3`,`image4`, `id_loai`) VALUES ('$tensp1','$price','$details','1','$hinhanh1','$hinhanh2','$hinhanh3','$hinhanh4','$theloai')";
        $result=$connect->query($sql);
        ...
        $result2=$connect->query($sql2);
        if($result2){
            move_uploaded_file($_FILES['upload_file1']['tmp_name'],'images/'.$_FILES['upload_file1']['name']);
            ?>
                    <script>
                        alert("Them san pham thanh cong");
                        window.location.href="index.php?action=quanlysp";
